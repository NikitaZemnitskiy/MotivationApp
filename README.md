...
**Стек:** Java 21, Spring Boot 3, Spring Security (Basic), статический UI (HTML/CSS/JS), JSON-файл для хранения состояния. Используются Lombok и Java Records для сокращения шаблонного кода.
...
## Хранилище
Все данные пишутся в `data/app-state.json` (путь настраивается в `application.yml`). Файл создаётся автоматически при первом запуске.

## Возможности
- Ежедневные задания (минутные и «чек-бокс»).
- Стрики для спорта, английского и вьетнамских слов.
...
- Роли: USER видит и отмечает задания, ADMIN управляет магазином/целями и может добавлять простые ежедневные задачи (с опцией стрика).
